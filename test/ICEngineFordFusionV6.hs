...
module ICEngineFordFusionV6 where
...
getGearRatioUsed mph rpm =
    let gb = FordFusionV6.gearbox
        diff = FordFusionV6.diff
        --
        vSpeed = fSpeedToMeterSecond $ MileHour mph
        wRevs = wheelSpeed vSpeed FordFusionV6.wheelRadius
        --
        allGearsScore =
            [ (gr, score)
            | gr <- gb
...
tEngineTorque t =
    let Meter wRad = fLengthToMeter FordFusionV6.wheelRadius
        dTorque = Nm $ tdynoTorque t * wRad
        gr = getGearRatioUsed (tdynoSpeed t) (tengineSpeed t)
        --
        transmission = [gr, FordFusionV6.diff]
        --
...
testKNN = do
    ...
    --
    makeChart "HTMLS/FordFusionV6_KNN.html" "FuelFlow vs Power" [MyScatter originalPlot, MyScatter p1, MyLine myps, MyScatter tmpPlot]
    ...
    -- Residual Plots
    makeChart "HTMLS/FordFusionV6_KNN_Residuals.html" "Power vs Residuals" [MyScatter $ Prelude.zip (Prelude.map snd originalPlot) (standardizedResiduals diffs)]
...
testRegression = do
    ...
    makeChart "HTMLS/FordFusionV6_ICPUEM.html" "Power vs AirFlow" [MyScatter fuelFlowVsPower, MyScatter p1, MyScatter myps]
    --
    -- Print MSEs.
    let diffs =
            [ diff
    ...
    -- Residual Plots
    makeChart "HTMLS/FordFusionV6_ICPUEM_Residuals.html" "Power vs Residuals" [MyScatter $ Prelude.zip (Prelude.map snd fuelFlowVsPower) (standardizedResiduals diffs)]
...
filterNeg5 xs = [(a, b, c, d, e) | (a, b, c, d, e) <- xs, a > 10 && a < 200 && b >= 1000 && c > 0 && e > 0]
...
readAll f = do
            d1 <- getVals f "../data/FordFusionV6/1_Data.csv"
            d2 <- getVals f "../data/FordFusionV6/2_Data.csv"
            d3 <- getVals f "../data/FordFusionV6/3_Data.csv"
            d4 <- getVals f "../data/FordFusionV6/4_Data.csv"
            d5 <- getVals f "../data/FordFusionV6/5_Data.csv"
            --
